...
jest.mock('routes/guards/allResponseTasksCompletedGuard', () => ({
  AllResponseTasksCompletedGuard: {
    apply: () => (_req: Request, _res: Response, next: NextFunction) => next(),
  },
}));

jest.mock('routes/guards/pcqGuard', () => ({
  isFirstTimeInPCQ: (_req: Request, _res: Response, next: NextFunction) => next(),
}));
...
const SUBMITTED_DATE = new Date('2026-01-02T09:00:00.000Z');
