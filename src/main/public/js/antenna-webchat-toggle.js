(function() {
  ...
  webChat.addEventListener('metrics', function(metrics) {
    ...
    if (ccState != 'Open' || (today.getHours() < 9 || today.getHours() > 17)) {
      ...
      document.getElementById('metrics').textContent = 'No Advisors are currently available, Come back later.';
      button.classList.add('hidden');
      buttonChat.classList.add('hidden');
    } else if (waitTimeInMinutes > 5 && waitTimeInMinutes < 60) {
      document.getElementById('metrics').textContent = 'All our Advisors are busy helping other people. Please try again later or contact us by phone or email.';
      button.classList.add('hidden');
      buttonChat.classList.add('hidden');
    } else {
