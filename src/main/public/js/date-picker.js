const datePickerUtils = {
  ...
  getDisplayValueOfDate: function(element) {
...
const datePicker = {
  ...
  init: function() {
    return datePicker.getBankHolidays(function(bankHolidays) {
      return datePicker.buildDatePicker(bankHolidays);
    });
  },
  ...
  getBankHolidays: function(callback) {
    $.ajax({
      ...
      success: function(res) {
        ...
        const dates = events.map(function(event) {
          return moment(event.date).format('MM-D-YYYY');
        });
        callback(dates);
      },
      error: function() {
  ...
  setUpDOWHeading: function() {
    ...
    const dow = $('.dow');
    $.each(dow, function(index, node) {
      return node.textContent = days[index];
    });
  },

  toggleArrows: function(nextOrPrevArrow) {
    return '<img tabindex=0  alt="date picker' + nextOrPrevArrow + '" src="/img/date-picker/' + nextOrPrevArrow + '_arrow.png" />';
  },

  changeDateHandler: function(event) {
    const uuid = $('#externalId')[0].innerText;
    const csrf = $('input[name="_csrf"]').val();
    let dates = event.dates.map(function(eventDate) {
      return datePickerUtils.formatDateForData(eventDate);
    });
        ...
        $('#date-selection-wrapper .add-another-delete-link').click(function(e) {
          ...
          const d = dates
            .map(function(localDate) {
              return moment({ year: localDate.year, month: localDate.month - 1, day: localDate.day });
            })
            .map(function(mDate) {
              return mDate.toDate();
            })
            ...
            .filter(function(localDate, index) {
              return index !== Number(dateIndex);
            });
          datePicker.selector().datepicker('setDates', d);
        });
      });
