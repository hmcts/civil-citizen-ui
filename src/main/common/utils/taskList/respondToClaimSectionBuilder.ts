...
const buildPartialAdmissionTasks = (caseData: Claim, claimId: string, lang: string, tasks: RespondToClaimTasks): Task[] => {
  ...
  if (isPartialAdmissionBySetDate(caseData)) {
  ...
  // Keep original ordering semantics from splice(2, 0, ...)
  partialAdmissionTasks.splice(1, 0, whyDisagreeWithAmountClaimedTask);
