...
app.use((req, _res, next) => {
  ...
  console.log('cookie present?', Boolean(req.headers.cookie?.includes('citizen-ui-session=')), req.url);
  // logger.info('cookie present?', Boolean(req.headers.cookie?.includes('citizen-ui-session=')), req.url);
  next();
});
