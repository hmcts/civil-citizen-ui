...
import {
  getApplyHelpWithApplicationFeeContents, getButtonsContents,
} from 'services/features/generalApplication/applicationFee/helpWithApplicationFeeContent';
import {saveHelpApplicationFeeSelection} from 'services/features/generalApplication/generalApplicationService';
...
async function renderView(res: Response, req: AppRequest | Request, form: GenericForm<GenericYesNo>, claimId: string, redirectUrl: string, lng: string) {
  if (!form) {
    ...
    form = new GenericForm(new GenericYesNo(claim.generalApplication?.applyHelpWithFees));
  }
  res.render(applyHelpWithApplicationFeeViewPath,
    {
      ...
      form,
      backLinkUrl,
      applyHelpWithFeeSelectionContents: getApplyHelpWithApplicationFeeContents(lng),
...
helpWithApplicationFeeController.post(GA_APPLY_HELP_WITH_FEE_SELECTION, (async (req: AppRequest | Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (form.hasErrors()) {
      ...
      await saveHelpApplicationFeeSelection(redisKey, claim, req.body.option);
      const redirectUrl = await getRedirectUrl(claimId, claim.generalApplication?.applyHelpWithFees, <AppRequest>req);
