...
import {buildClaimantNotifications, getDashboardTaskList} from 'services/dashboard/getDashboardContent';
...
claimantDashboardController.get(DASHBOARD_CLAIMANT_URL, async (req: Request, res: Response, next: NextFunction) => {
  try {
      ...
      const dashboardNotifications = buildClaimantNotifications(claim, lang);
