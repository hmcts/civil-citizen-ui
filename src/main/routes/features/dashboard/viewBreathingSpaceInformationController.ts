...
import {
  BREATHING_SPACE_INFO, DASHBOARD_CLAIMANT_URL,
} from '../../urls';
...
async function renderView(req: Request, res: Response, claim: Claim, claimId: string) {
  ...
  const pageTitle =  'PAGES.DASHBOARD.SUPPORT_LINKS.GET_DEBT_RESPITE';
...
viewBreathingSpaceInformationController.get(BREATHING_SPACE_INFO, (async (req: Request, res: Response, next: NextFunction) => {
  try {
