...
async function renderView(req: Request, res: Response, claimId: string) {
  const lng = req.query.lang ? req.query.lang : req.cookies.lang;
  const contentList = getBSGuidanceContent(lng, constructResponseUrlWithIdParams(claimId, QM_START_URL)+'?linkFrom=start');
