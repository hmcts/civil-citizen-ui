...
dashboardController.get(DASHBOARD_URL, async function (req, res) {
  ...
  const claimsAsClaimant : DashboardClaimantItem[] = await getClaimsForClaimant(appRequest);
  const claimsAsDefendant : DashboardDefendantItem[] = await getClaimsForDefendant(appRequest);
