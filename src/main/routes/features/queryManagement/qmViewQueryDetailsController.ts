...
const renderView = async (res: Response, claimId: string, claim: Claim, lang: string, selectedQueryItem: QueryDetail, queryId: string): Promise<void> => {
...
qmViewQueryDetailsController.get(QM_QUERY_DETAILS_URL, (async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    await renderView(res, claimId, claim, lang, selectedQueryItem, queryId);
  } catch (error) {
    next(error);
  }
}) as RequestHandler);
