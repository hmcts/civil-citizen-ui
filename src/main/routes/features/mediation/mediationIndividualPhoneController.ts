...
async function renderView(form: GenericForm<MediationIndividualPhoneNumber>, res: Response, claimId: string): Promise<void> {
  ...
  res.render(mediationIndividualPhoneViewPath, { form, respondentTelNumber: (claim.isClaimantIntentionPending() ? claim.applicant1?.partyPhone?.phone : claim.respondent1?.partyPhone?.phone), pageTitle:'PAGES.MEDIATION_CAN_WE_USE.PAGE_TITLE' });
}
