...
const emailMediationConfirmationViewPath = 'features/mediation/mediation-disagreement';
...
function renderView(form: GenericForm<GenericYesNo>, res: Response, claimStatus: CaseState): void {
  res.render(emailMediationConfirmationViewPath, {form, claimStatus});
}
...
mediationDisagreementController.get(MEDIATION_DISAGREEMENT_URL, async (req, res, next: NextFunction) => {
  try {
    ...
    const emailMediationForm = new GenericForm(new GenericYesNo(mediation.isMediationEmailCorrect?.option));
    renderView(emailMediationForm, res, claim.ccdState);
  } catch (error) {
    next(error);
  }
});
