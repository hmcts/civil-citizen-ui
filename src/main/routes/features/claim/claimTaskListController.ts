...
claimTaskListController.get(CLAIMANT_TASK_LIST_URL, claimIssueTaskListGuard, (async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const title = completed < total
      ? t('PAGES.CLAIM_TASK_LIST.APPLICATION_INCOMPLETE', {lng})
      : t('PAGES.CLAIM_TASK_LIST.APPLICATION_COMPLETE', {lng});
    res.render(taskListViewPath, {
      taskLists,
      title,
      description,
