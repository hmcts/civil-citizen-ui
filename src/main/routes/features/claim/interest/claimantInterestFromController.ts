...
claimantInterestFromController.get(CLAIM_INTEREST_FROM_URL, async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const form: InterestClaimFromSelection = await getClaimantInterestFrom(claimId);
    renderView(new GenericForm<InterestClaimFromSelection>(form),res);
  } catch (error) {
    next(error);
  }
});
...
claimantInterestFromController.post(CLAIM_INTEREST_FROM_URL, async (req: any, res: Response, next: NextFunction) => {
  try {
    ...
    if (form.hasErrors()) {
      ...
      await saveClaimantInterestFrom(claimId, form.model);
