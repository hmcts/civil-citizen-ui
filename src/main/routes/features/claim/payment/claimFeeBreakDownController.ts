...
claimFeeBreakDownController.get(CLAIM_FEE_BREAKUP, claimFeePaymentGuard, (async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    return res.render(viewPath, { totalClaimAmount: claim.totalClaimAmount?.toFixed(2), interest: interestAmount, claimFee, hasInterest, totalAmount: totalAmount?.toFixed(2), paymentSyncError, pageTitle: 'PAGES.FEE_AMOUNT.TITLE' });
  } catch (error) {
    next(error);
  }
})as RequestHandler);
...
claimFeeBreakDownController.post(CLAIM_FEE_BREAKUP, (async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    const paymentRedirectInformation = await getRedirectInformation(req, claimId);
...
async function getRedirectInformation(req: AppRequest, claimId: string) {
  try {
    const paymentRedirectInformation = await getFeePaymentRedirectInformation(
      claimId,
      FeeType.CLAIMISSUED,
      req,
    );
    return paymentRedirectInformation;
  } catch (error) {
    const claim = await getClaimById(req.params.id, req, true);
