...
const interestTypeViewPath = 'features/response/citizenDob/citizen-dob';

interestTypeController.get(CLAIMANT_DOB_URL, async (req: AppRequest, res: express.Response, next: express.NextFunction) => {
  try {
    ...
    res.render(interestTypeViewPath, {form, today: new Date(), claimantView: true});
  } catch (error) {
    next(error);
  }
});

interestTypeController.post(CLAIMANT_DOB_URL, async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  try {
