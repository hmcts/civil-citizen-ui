...
claimantDoBController.get(CLAIMANT_DOB_URL, (async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    res.render(claimantDoBViewPath, {form, today: new Date(), claimantView: true, maxDateForAge18: getDOBforAgeFromCurrentTime(18), pageTitle: 'PAGES.CLAIMANT_DOB.PAGE_TITLE'});
  } catch (error) {
    next(error);
  }
}) as RequestHandler);
