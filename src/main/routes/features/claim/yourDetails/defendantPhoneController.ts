...
defendantPhoneController.get(CLAIM_DEFENDANT_PHONE_NUMBER_URL, (async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    const form: CitizenTelephoneNumber = await getTelephone(req, ClaimantOrDefendant.DEFENDANT);
...
defendantPhoneController.post(CLAIM_DEFENDANT_PHONE_NUMBER_URL, (async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    if (form.hasErrors()) {
      ...
      await saveTelephone(req, form.model, ClaimantOrDefendant.DEFENDANT);
