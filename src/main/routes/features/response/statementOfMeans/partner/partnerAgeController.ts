...
function renderView(partnerAge: GenericForm<PartnerAge>, res: express.Response): void {
...
partnerAgeController.post(CITIZEN_PARTNER_AGE_URL,
  async (req, res, next: express.NextFunction) => {
    try {
      const form: GenericForm<PartnerAge> = new GenericForm(new PartnerAge(req.body.option));
      form.validateSync();
      if (form.hasErrors()) {
        renderView(form, res);
      } else {
