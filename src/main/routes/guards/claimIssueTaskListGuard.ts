...
import {BASE_ELIGIBILITY_URL, CLAIM_CONFIRMATION_URL} from 'routes/urls';
...
export const claimIssueTaskListGuard = (async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!caseData?.isDraftClaim() && !req.cookies['eligibilityCompleted'] && !req.originalUrl.includes(CLAIM_CONFIRMATION_URL)) {
