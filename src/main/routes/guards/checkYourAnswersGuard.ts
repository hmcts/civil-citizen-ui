...
export const checkYourAnswersClaimGuard = async (req: AppRequest, res: Response, next: NextFunction) => {
  try {
    ...
    // doNotThrowError: a draft that has already been submitted (and therefore deleted from
    // the draft store) returns an empty claim rather than throwing 'Case not found'. This keeps
    // the check-and-send GET idempotent when a claim is submitted and the page is re-entered
