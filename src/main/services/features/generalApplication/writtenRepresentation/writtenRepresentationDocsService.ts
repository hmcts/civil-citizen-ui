...
export const buildSummarySection = (writtenRepText: string, uploadDocumentsList: UploadGAFiles[], claimId: string, appId: string, lng: any) => {
  ...
  if (uploadDocumentsList.length > 0) {
    ...
    rowValueDoc += '</ul>';
    rows.push(summaryRowHtml(t('PAGES.GENERAL_APPLICATION.UPLOAD_MORE_INFO_DOCUMENTS.UPLOAD_DOC_CYA_TITLE', {lng}), rowValueDoc, constructResponseUrlWithIdAndAppIdParams(claimId, appId, GA_UPLOAD_WRITTEN_REPRESENTATION_DOCS_URL), changeLabel()));
  }
  return rows;
};
