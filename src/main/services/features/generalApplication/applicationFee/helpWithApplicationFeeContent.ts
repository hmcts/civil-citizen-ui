...
export const getApplicationFeeContentPageDetails = (claimId: string, genAppId: string) => {
  const nextPageUrl = GA_APPLY_HELP_WITH_FEE_REFERENCE.replace(':id', claimId) + (genAppId ? `?id=${genAppId}` : '');
