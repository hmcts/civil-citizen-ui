...
import {ConfirmYouHaveBeenPaidSectionBuilder} from 'models/judgmentOnline/confirmYouHaveBeenPaidSectionBuilder';
import {constructResponseUrlWithIdParams} from 'common/utils/urlFormatter';
import {t} from 'i18next';

const CONFIRM_YOU_HAVE_BEEN_PAID = 'PAGES.CONFIRM_YOU_HAVE_BEEN_PAID';

export const getConfirmYouHaveBeenPaidContents = (claimId: string, claim: Claim) => {
  const afterTheDeadline=t(`${CONFIRM_YOU_HAVE_BEEN_PAID}.AFTER_THE_DEADLINE`);
  const linkParagraph = `<p class="govuk-body govuk-!-margin-bottom-1">${afterTheDeadline}
        <a target="_blank" class="govuk-link" rel="noopener noreferrer" href=${t(MAKE_APPLICATION_TO_COURT)}>${t(`${CONFIRM_YOU_HAVE_BEEN_PAID}.APPLY_TO_THE_COURT`)}</a>
        ${t(`${CONFIRM_YOU_HAVE_BEEN_PAID}.IF_YOU_WANT_ANY`)}
    </p><br>`;

  return new ConfirmYouHaveBeenPaidSectionBuilder()
