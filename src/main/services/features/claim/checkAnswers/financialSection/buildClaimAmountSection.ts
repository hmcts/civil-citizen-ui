...
export const buildClaimAmountSection = (claim: Claim, lang: string): SummarySection => {
  ...
  if (claim?.isSameRateTypeEightPercent() && !claim?.isBreakDownCompleted()) {
    ...
    claimAmountSection.summaryList.rows.push(summaryRowWithTextValue(t('PAGES.CLAIMANT_INTEREST_RATE.TITLE', {lng}), differentRateInterestType, CLAIM_INTEREST_RATE_URL, changeLabel(lang)));
    claimAmountSection.summaryList.rows.push(summaryRowWithTextValue(t('PAGES.CLAIMANT_INTEREST_RATE.REASON', {lng}), claim.interest?.sameRateInterestSelection.reason, CLAIM_INTEREST_RATE_URL, changeLabel(lang)));
  }
  if (claim?.interest?.interestClaimFrom) {
    ...
    if (claim?.isInterestFromASpecificDate() && !claim?.isBreakDownCompleted()) {
      ...
      claimAmountSection.summaryList.rows.push(summaryRowWithTextValue(t('PAGES.CLAIM_JOURNEY.WHEN_CLAIM_INTEREST_FROM.REASON', {lng}), claim.interest.interestStartDate.reason, CLAIM_INTEREST_START_DATE_URL, changeLabel(lang)));
  ...
  if (claim.claimDetails?.helpWithFees) {
    ...
    claimAmountSection.summaryList.rows.push(summaryRowWithTextValue(t('PAGES.HELP_WITH_FEES.REFERENCE_NUMBER', {lng}), hwfReferenceNumber, CLAIM_HELP_WITH_FEES_URL, changeLabel(lang)));
  }
