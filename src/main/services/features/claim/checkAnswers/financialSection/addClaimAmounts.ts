...
export const addClaimAmounts = (claim: Claim, claimAmountSection: SummarySection, lang: string ) => {
  if (claim.claimAmountBreakup) {
    ...
    for (const element of claimAmounts) {
      claimAmountSection.summaryList.rows.push(
        summaryRow(element.value.claimReason, currencyFormatWithNoTrailingZeros(Number(element.value.claimAmount)), CLAIM_AMOUNT_URL, changeLabel(lang)),
      );
    }
  }
};
