...
export const buildExpertReportSection = (section: ExpertSection = null, index = 0, form: GenericForm<UploadDocumentsUserForm> = null): ClaimSummarySection[] => {
  ...
  return new UploadDocumentsSectionBuilder()
    ...
    .addDateArray('PAGES.UPLOAD_DOCUMENTS.EXPERT.DATE_REPORT_WAS', invalidDateErrors, 'PAGES.UPLOAD_DOCUMENTS.DATE_EXAMPLE', expertReport, 'dateInputFields', 'date', section?.dateInputFields?.dateDay.toString(), section?.dateInputFields?.dateMonth.toString(), section?.dateInputFields?.dateYear.toString(), index)
    .addUploadArray('PAGES.UPLOAD_DOCUMENTS.UPLOAD', '', expertReport, fileUpload, index, section?.fileUpload?.fieldname, invalidDateErrors.invalidFileError, section?.caseDocument)
...
export const buildJointStatementSection = (section: ExpertSection = null, index = 0, form: GenericForm<UploadDocumentsUserForm> = null): ClaimSummarySection[] => {
  ...
  return new UploadDocumentsSectionBuilder()
    ...
    .addDateArray('PAGES.UPLOAD_DOCUMENTS.DATE', invalidDateErrors, 'PAGES.UPLOAD_DOCUMENTS.DATE_EXAMPLE', expertStatement, 'dateInputFields', 'date', section?.dateInputFields?.dateDay.toString(), section?.dateInputFields?.dateMonth.toString(), section?.dateInputFields?.dateYear.toString(), index)
    .addUploadArray('PAGES.UPLOAD_DOCUMENTS.UPLOAD', '', expertStatement, fileUpload, index, section?.fileUpload?.fieldname, invalidDateErrors.invalidFileError, section?.caseDocument)
    .addRemoveSectionButton(form?.model.expertStatement?.length > 1 || false)
    .build();
};

export const buildQuestionsForOtherSection = (selectItems: ({
  text: string;
  value: string
})[], section: ExpertSection = null, index = 0, form: GenericForm<UploadDocumentsUserForm> = null): ClaimSummarySection[] => {
  const errorFieldNamePrefix = `${questionsForExperts}[${questionsForExperts}][${index}]`;
  const invalidDateErrors = {
    invalidFileError: form?.errorFor(`${errorFieldNamePrefix}[fileUpload]`, questionsForExperts)
      || form?.errorFor(`${errorFieldNamePrefix}[fileUpload][mimetype]`, questionsForExperts)
      || form?.errorFor(`${errorFieldNamePrefix}[fileUpload][size]`, questionsForExperts),
  };
  return new UploadDocumentsSectionBuilder()
    ...
    .addUploadArray('PAGES.UPLOAD_DOCUMENTS.UPLOAD', '', questionsForExperts, fileUpload, index, section?.fileUpload?.fieldname, invalidDateErrors.invalidFileError, section?.caseDocument)
    .addRemoveSectionButton(form?.model.questionsForExperts?.length > 1 || false)
    .build();
};

export const buildAnswersToQuestionsSection = (selectItems: ({
  text: string;
  value: string
})[], section: ExpertSection = null, index = 0, form: GenericForm<UploadDocumentsUserForm> = null): ClaimSummarySection[] => {
  const errorFieldNamePrefix = `${answersForExperts}[${answersForExperts}][${index}]`;
  const invalidDateErrors = {
    invalidFileError: form?.errorFor(`${errorFieldNamePrefix}[fileUpload]`, answersForExperts)
      || form?.errorFor(`${errorFieldNamePrefix}[fileUpload][mimetype]`, answersForExperts)
      || form?.errorFor(`${errorFieldNamePrefix}[fileUpload][size]`, answersForExperts),
  };
  return new UploadDocumentsSectionBuilder()
    ...
    .addUploadArray('PAGES.UPLOAD_DOCUMENTS.UPLOAD', '', answersForExperts, fileUpload, index, section?.fileUpload?.fieldname, invalidDateErrors.invalidFileError, section?.caseDocument)
