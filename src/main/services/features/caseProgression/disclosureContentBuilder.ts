...
export const buildDisclosureListSection = (section: FileOnlySection = null, index = 0, form: GenericForm<UploadDocumentsUserForm> = null): ClaimSummarySection[] => {
  ...
  const invalidDateErrors = {
    invalidFileError: form?.errorFor(`${errorFieldNamePrefix}[fileUpload]`, disclosureList)
      || form?.errorFor(`${errorFieldNamePrefix}[fileUpload][mimetype]`, disclosureList)
      || form?.errorFor(`${errorFieldNamePrefix}[fileUpload][size]`, disclosureList),
  };
  return new UploadDocumentsSectionBuilder()
    .addTitle('PAGES.UPLOAD_DOCUMENTS.DISCLOSURE.DISCLOSURE_LIST', {}, 'govuk-!-width-three-quarters')
    .addUploadArray('PAGES.UPLOAD_DOCUMENTS.UPLOAD', '', disclosureList, fileUpload, index, section?.fileUpload?.fieldname, invalidDateErrors.invalidFileError, section?.caseDocument)
