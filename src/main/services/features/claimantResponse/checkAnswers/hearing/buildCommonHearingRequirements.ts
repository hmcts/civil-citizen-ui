...
export const getWitnesses = (claim: Claim, claimId: string, lng: string): SummaryRow[]  => {
  ...
  const otherWitnesses = claim.claimantResponse.directionQuestionnaire?.witnesses?.otherWitnesses?.option === YesNo.YES ? YesNoUpperCase.YES : YesNoUpperCase.NO;
  ...
  if(otherWitnesses === YesNoUpperCase.YES){
    const witnesses: OtherWitnessItems[] = claim?.claimantResponse?.directionQuestionnaire?.witnesses?.otherWitnesses?.witnessItems;
...
export const getSummaryRowForDisplayEvidenceYourself = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const giveEvidenceYourselfAnswer = getFormattedAnswerForYesNoNotReceived(claim.claimantResponse.directionQuestionnaire?.defendantYourselfEvidence?.option, lng);
...
export const vulnerabilityQuestion = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const option = claim.claimantResponse.directionQuestionnaire?.vulnerabilityQuestions?.vulnerability?.option === YesNo.YES
    ? YesNoUpperCase.YES
    : YesNoUpperCase.NO;
...
export const vulnerabilityInfo = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const details = claim.claimantResponse.directionQuestionnaire?.vulnerabilityQuestions?.vulnerability?.vulnerabilityDetails;
...
export const giveEvidenceYourself = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const option = claim.claimantResponse.directionQuestionnaire?.defendantYourselfEvidence?.option === YesNo.YES
    ? YesNoUpperCase.YES
    : YesNoUpperCase.NO;
...
export const speakingLanguagePreference = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const speakingLanguageName = getLanguageSelected(claim.claimantResponse.directionQuestionnaire?.welshLanguageRequirements?.language?.speakLanguage, lng);
...
export const documentsLanguagePreference = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  ...
  const documentsLanguageName = getLanguageSelected(claim.claimantResponse.directionQuestionnaire?.welshLanguageRequirements?.language?.documentsLanguage, lng);
...
export const phoneAndVideoQuestion = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const option =  claim.claimantResponse.directionQuestionnaire?.hearing?.phoneOrVideoHearing?.option === YesNo.YES
    ? YesNoUpperCase.YES
    : YesNoUpperCase.NO;
...
export const phoneAndVideoInfo = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const details = claim.claimantResponse.directionQuestionnaire?.hearing?.phoneOrVideoHearing?.details;
...
export const getUnavailableDatesList = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const hasUnavailableDatesForHearing = getListOfUnavailableDate(claim.claimantResponse.directionQuestionnaire?.hearing?.unavailableDatesForHearing, lng);
  return summaryRow(
...
export const getUnavailabilityReason = (claim: Claim, claimId: string, days: number, lng: string): SummaryRow => {
  const whyUnavailableForHearing = claim.claimantResponse.directionQuestionnaire.hearing.whyUnavailableForHearing?.reason;
  return summaryRow(
...
export const displayUnavailabilityForHearing = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const hasUnavailableDatesForHearing = getFormattedAnswerForYesNoNotReceived(claim.claimantResponse.directionQuestionnaire?.hearing?.cantAttendHearingInNext12Months?.option, lng);
  return summaryRow(
...
export const getSpecificCourtLocation = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const hasSpecificCourtLocation = getFormattedAnswerForYesNoNotReceived(claim.claimantResponse.directionQuestionnaire?.hearing?.specificCourtLocation?.option, lng);
  return summaryRow(
...
export const displaySpecificCourtLocation = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const hasSpecificCourtLocation = claim.claimantResponse.directionQuestionnaire?.hearing?.specificCourtLocation?.courtLocation;
  return summaryRow(
...
export const getSpecificCourtLocationReason = (claim: Claim, claimId: string, lng: string): SummaryRow => {
  const whySpecificCourtLocation = claim.claimantResponse.directionQuestionnaire?.hearing?.specificCourtLocation?.reason;
  return summaryRow(
...
export const buildCommonHearingRequirements = (claim: Claim, hearingRequirementsSection: SummarySection, claimId: string, lng: string) => {
  ...
  if (claim.claimantResponse.directionQuestionnaire?.defendantYourselfEvidence?.option) {
  ...
  if (claim.claimantResponse.directionQuestionnaire?.hearing?.cantAttendHearingInNext12Months?.option === YesNo.YES) {
    hearingRequirementsSection.summaryList.rows.push(getUnavailableDatesList(claim, claimId, lng));
    const numberOfUnavailableDays = getNumberOfUnavailableDays(claim.claimantResponse.directionQuestionnaire.hearing.unavailableDatesForHearing);
  ...
  if (claim.claimantResponse.directionQuestionnaire?.hearing?.phoneOrVideoHearing?.option) {
    hearingRequirementsSection.summaryList.rows.push(phoneAndVideoQuestion(claim, claimId, lng));

    if(claim.claimantResponse.directionQuestionnaire?.hearing?.phoneOrVideoHearing?.option === YesNo.YES)
      hearingRequirementsSection.summaryList.rows.push(phoneAndVideoInfo(claim, claimId, lng));
  }

  if (claim.claimantResponse.directionQuestionnaire?.vulnerabilityQuestions?.vulnerability) {
    hearingRequirementsSection.summaryList.rows.push(vulnerabilityQuestion(claim, claimId, lng));

    if(claim.claimantResponse.directionQuestionnaire?.vulnerabilityQuestions?.vulnerability?.option === YesNo.YES)
  ...
  if (claim.claimantResponse.directionQuestionnaire?.hearing?.specificCourtLocation?.option === YesNo.YES) {
  ...
  if (claim.claimantResponse.directionQuestionnaire?.welshLanguageRequirements?.language) {
