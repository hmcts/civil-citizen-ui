...
export const determinationWithoutHearingQuestion = (claim: Claim, claimId: string, lng: string) => {
  ...
  const determinationWithoutHearingOption = claim.claimantResponse.directionQuestionnaire?.hearing?.determinationWithoutHearing?.option === YesNo.YES
    ? YesNoUpperCase.YES
    : YesNoUpperCase.NO;
...
export const determinationWithoutHearingReason = (claim: Claim, claimId: string, lng: string) => {
  const reason = claim.claimantResponse.directionQuestionnaire?.hearing?.determinationWithoutHearing?.reasonForHearing;
...
export const buildSmallClaimHearingRequirements = (claim: Claim, hearingRequirementsSection: SummarySection, claimId: string, lng: string) => {
  ...
  if (claim.claimantResponse.directionQuestionnaire?.hearing?.determinationWithoutHearing?.option === YesNo.NO) {
