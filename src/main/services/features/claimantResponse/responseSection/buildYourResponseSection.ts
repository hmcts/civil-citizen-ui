...
function buildFDDisputeTheClaimSummaryRows(claim: Claim, claimId: string, lang : string | unknown) {
  ...
  return summaryRow(t('PAGES.CHECK_YOUR_ANSWER.PROCEED_WITH_CLAIM', {lang}), t(`COMMON.${intentionToProceed}`, {lang}), intentionToProceedHref, changeLabel(lang));
}
...
function buildPartAdmitPayImmediatelySummaryRows(claim: Claim, claimId: string, lang : string | unknown) {
  ...
  return summaryRow(t('PAGES.CHECK_YOUR_ANSWER.DO_YOU_ACCEPT_OR_REJECT_THE_DEFENDANTS_ADMISSION', { lang }), t(RESPONSEFORNOTPAIDPAYIMMEDIATELY[selectedOption], { lang }), partAdmitAcceptedHref, changeLabel(lang));
}

export const buildYourResponseSection = (claim: Claim, claimId: string, lang: string | unknown): SummarySection => {
  const yourResponseToClaimSection = summarySection({
