...
export const getJudgmentAmountSummary = (claim: Claim, claimFee: number, lang: string) => {
  ...
  const hasDefendantAlreadyPaid = claimantResponse.hasDefendantPaid;
  const alreadyPaidAmount = hasDefendantAlreadyPaid ? claimantResponse.getDefendantPaidAmount : 0;
