...
const saveClaimantResponse = async (claimId: string, value: any, claimantResponsePropertyName: string, parentPropertyName?: string): Promise<void> => {
  try {
    ...
    if (claimantResponse.hasClaimantRejectedDefendantPaid()) {
    ...
    if (claimantResponse.hasClaimantSettleTheClaimForDefendantPartlyPaidAmount() || claimantResponse.hasClaimantAcceptedDefendantResponse()) {
