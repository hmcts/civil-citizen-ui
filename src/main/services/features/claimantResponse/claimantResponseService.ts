  ...
  if (claim.statementOfMeans?.bankAccounts) {
    ...
    bankAccounts.forEach((account) => {
      ...
      },
      {
        ...
        value: {
          text: account?.joint === 'yes' ? t('COMMON.VARIATION_4.YES', {lng}) : t('COMMON.VARIATION_4.NO', {lng}),
        },
      });
    });
  } else {
...
const constructFinancialSupportSection = (claim: Claim, lng: string) => {
  ...
  sectionRows.push({
    ...
    value: {
      text: otherDependants ? t('COMMON.VARIATION_4.YES', {lng}) : t('COMMON.VARIATION_4.NO', {lng}),
    },
    classes: 'govuk-summary-list__row--no-border',
  });
