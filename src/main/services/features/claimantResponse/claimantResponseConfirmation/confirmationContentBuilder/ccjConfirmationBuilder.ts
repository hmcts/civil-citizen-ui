...
export const getCCJNextStepsForJudgeDecideRepaymentPlan = (claim: Claim, lng: string) => {
  const defendantName = claim.getDefendantFullName();
  return new PageSectionBuilder()
    .addTitle('PAGES.SUBMIT_CONFIRMATION.WHAT_HAPPENS_NEXT', {lng})
    .addParagraph('PAGES.CLAIMANT_RESPONSE_CONFIRMATION.CCJ.CCJ_NEXT_STEP_MSG4', {defendantName, lng})
    .addParagraph('PAGES.CLAIMANT_RESPONSE_CONFIRMATION.CCJ.CCJ_NEXT_STEP_MSG2', {defendantName, lng})
    .addParagraph('PAGES.CLAIMANT_RESPONSE_CONFIRMATION.CCJ.CCJ_NEXT_STEP_MSG5', {defendantName, lng})
    .addParagraph('PAGES.CLAIMANT_RESPONSE_CONFIRMATION.CCJ.CCJ_NEXT_STEP_MSG6', {defendantName, lng})
    .build();
};
