  ...
  if (ccdCourtOrders?.length)
  {
    return ccdCourtOrders.map((ccdCourtOrder: CCDCourtOrders) => {
      return new CourtOrder(
        ccdCourtOrder.value?.amountOwed? ccdCourtOrder.value?.amountOwed/100 : undefined,
        ccdCourtOrder.value?.monthlyInstalmentAmount? ccdCourtOrder.value?.monthlyInstalmentAmount/100 : undefined,
