...
export const toCCDParty = (party: Party): CCDParty => {
  return {
    ...
    individualDateOfBirth: party?.type === PartyType.INDIVIDUAL ? party?.dateOfBirth?.toString().substring(0,10) : null,
    ...
    soleTraderDateOfBirth: party?.type === PartyType.SOLE_TRADER ? party?.dateOfBirth?.toString().substring(0,10) : null,
