...
const setDashboardValues = (claim: Claim, claimId: string, notificationId?: string): Map<string, string> => {
  ...
  valuesMap.set('{DOWNLOAD_SETTLEMENT_AGREEMENT}', CASE_DOCUMENT_DOWNLOAD_URL.replace(':id', claimId).replace(':documentId', getSystemGeneratedCaseDocumentIdByType(claim.systemGeneratedCaseDocuments, DocumentType.SETTLEMENT_AGREEMENT)));  valuesMap.set('{MEDIATION}', MEDIATION_SERVICE_EXTERNAL);
