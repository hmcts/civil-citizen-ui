/**
 * Dependent UI scenario files (relative paths using forward slashes).
 * Add file paths here when scenarios in the file must run sequentially as one journey.
 */
module.exports = [
  'src/test/functionalTests/tests/ui_tests/mediation/LiPvLiP_mediation_company_small_claims_tests.js',
  'src/test/functionalTests/tests/ui_tests/mediation/LiPvLiP_Individual_small_claims_tests.js',
  'src/test/functionalTests/tests/ui_tests/case_offline/LiPvLiP_case_offline_tests.js',
  'src/test/functionalTests/tests/ui_tests/case_offline/LRvLiP_case_offline_tests.js',
  'src/test/functionalTests/tests/ui_tests/part_admit/LipvLip_UI_PartAdmit_PaySetDate_SSA_tests.js',
  'src/test/functionalTests/tests/ui_tests/query_management/qm__LiPvLiP_ApplyToCourt_tests.js',
  'src/test/functionalTests/tests/ui_tests/query_management/qm_Hearing_LiPvLiP_tests.js',
  'src/test/functionalTests/tests/ui_tests/reject_all/LipvLip_UI_RejectAll_DisputeAll_Mediation_tests.js',
  'src/test/functionalTests/tests/ui_tests/welsh/welshResponse_caseworker_request_translation_tests.js',
  'src/test/functionalTests/tests/ui_tests/query_management/qm_nonHearing_LiPvLiP_tests.js',
];
