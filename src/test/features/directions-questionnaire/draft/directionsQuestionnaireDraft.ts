...
describe('DirectionsQuestionnaireDraft', () => {
  describe('deserialization', () => {
    ...
    it('should return a DirectionsQuestionnaireDraft instance initialised with valid data', () => {
      ...
      expect(directionsQuestionnaireDraftSampleData.expertReports.rows[0].reportDate).to.deep.equal({
        year: 2018,
        month: 1,
        day: 10,
      });
      expect(directionsQuestionnaireDraftSampleData.expertReports.rows[1].expertName).to.equal('Mr Rubeus Hagrid');
      expect(directionsQuestionnaireDraftSampleData.expertReports.rows[1].reportDate).to.deep.equal({
        year: 2019,
        month: 2,
        day: 27,
      });
