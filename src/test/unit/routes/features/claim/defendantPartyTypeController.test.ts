...
describe('Defendant Party Type Controller', () => {
  ...
  describe('on GET', () => {
    it('should return party type page', async () => {
      ...
      await request(app)
        ...
        .expect((res: request.Response) => {
          ...
          expect(res.text).toContain(t('PAGES.PARTY_TYPE.DEFENDANT_TITLE'));
        });
    });
  ...
  describe('on POST', () => {
    it('should redirect to individual details page when INDIVIDUAL selected', async () => {
      mockGetDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockGetCaseDataFromStore.mockResolvedValue(new Claim());
      mockUpdateDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      ...
      await request(app)
        ...
        .expect((res: request.Response) => {
          ...
          expect(res.header.location).toBe(CLAIM_DEFENDANT_INDIVIDUAL_DETAILS_URL);
        });
    });

    it('should redirect to sole trader details page when SOLE_TRADER selected', async () => {
      mockGetDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockGetCaseDataFromStore.mockResolvedValue(new Claim());
      mockUpdateDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockSaveDraftClaim.mockResolvedValue(undefined);
      ...
      await request(app)
        ...
        .expect((res: request.Response) => {
          expect(res.status).toBe(302);
          expect(res.header.location).toBe(CLAIM_DEFENDANT_SOLE_TRADER_DETAILS_URL);
        });
    });

    it('should redirect to company details page when COMPANY selected', async () => {
      mockGetDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockGetCaseDataFromStore.mockResolvedValue(new Claim());
      mockUpdateDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockSaveDraftClaim.mockResolvedValue(undefined);
    ...
    it('should redirect to organisation details page when ORGANISATION selected', async () => {
      mockGetDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockGetCaseDataFromStore.mockResolvedValue(new Claim());
      mockUpdateDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockSaveDraftClaim.mockResolvedValue(undefined);
      ...
      await request(app)
        ...
        .expect((res: request.Response) => {
          expect(res.status).toBe(302);
          expect(res.header.location).toBe(CLAIM_DEFENDANT_ORGANISATION_DETAILS_URL);
        });
    });

    it('should return errors on empty option', async () => {
      mockGetDraftClaim.mockResolvedValue(createMockManagerResult(new Claim()));
      mockGetCaseDataFromStore.mockResolvedValue(new Claim());
      ...
      await request(app)
        ...
        .expect((res: request.Response) => {
          expect(res.status).toBe(200);
          expect(res.text).toContain(t('ERRORS.VALID_CHOOSE_DEFENDANT_TYPE'));
        });
    });
