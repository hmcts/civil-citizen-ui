...
import {CLAIM_CLAIMANT_SOLE_TRADER_DETAILS_URL, CLAIMANT_PHONE_URL} from '../../../../../main/routes/urls';
...
describe('Claimant Sole Trader Details page', () => {
  ...
  describe('on Exception', () => {
    it('should return http 500 when has error in the get method', async () => {
      ...
      await request(app)
        .get(CLAIM_CLAIMANT_SOLE_TRADER_DETAILS_URL)
        .expect((res) => {
  ...
  it('get/Claimant sole trader details - should return test variable when there is no data on redis and civil-service', async () => {
    mockGetCaseData.mockImplementation(async () => {
      return new Respondent();
    });
    await request(app)
      .get('/claim/claimant-sole-trader-details')
      .expect((res) => {
        expect(res.status).toBe(200);
        expect(res.text).toContain('Enter your details');
      });
  ...
  it('POST/Claimant Sole Trader details - should redirect on correct primary address', async () => {
    await request(app)
      .post(CLAIM_CLAIMANT_SOLE_TRADER_DETAILS_URL)
      .send({
        primaryAddressLine1: 'Flat 3A Middle Road',
  ...
  it('POST/Claimant Sole Trader details - should return error on empty primary city', async () => {
    await request(app)
      .post(CLAIM_CLAIMANT_SOLE_TRADER_DETAILS_URL)
      .send({
        primaryAddressLine1: 'Flat 3A Middle Road',
  ...
  it('POST/Claimant Sole Trader details - should return error on empty primary postcode', async () => {
    await request(app)
      .post(CLAIM_CLAIMANT_SOLE_TRADER_DETAILS_URL)
      .send({
        primaryAddressLine1: 'Flat 3A Middle Road',
  ...
  it('POST/Claimant Sole Trader details - should return error on input for primary address when provideCorrespondenceAddress is set to NO', async () => {
    await request(app)
      .post(CLAIM_CLAIMANT_SOLE_TRADER_DETAILS_URL)
      .send({
        primaryAddressLine1: '',
  ...
  it('should redirect to claimant phone screen', async () => {
    ...
    await request(app)
      ...
      .expect((res) => {
        expect(res.status).toBe(302);
        expect(res.header.location).toEqual(CLAIMANT_PHONE_URL);
      });
  });
});
