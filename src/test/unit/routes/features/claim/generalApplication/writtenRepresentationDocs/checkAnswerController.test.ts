...
describe('General Application - additional docs check answer controller ', () => {
  ...
  describe('POST /chec-and-send', () => {
    it('should submit the documents and redirect to submitted page', async () => {
      ...
      expect(res.header.location).toBe(constructResponseUrlWithIdAndAppIdParams(claimId, gaId, GA_UPLOAD_WRITTEN_REPRESENTATION_DOCS_SUBMITTED_URL));
      expect(translateCUItoCCD).toHaveBeenCalledWith(uploadDocuments);
      expect(mockGaServiceClient).toHaveBeenCalledWith(ApplicationEvent.RESPOND_TO_JUDGE_WRITTEN_REPRESENTATION, gaId, { generalAppWrittenRepUpload: [] }, expect.anything());
    });
