...
import paymentDateController from '../../../../../../../main/routes/features/response/admission/partialAdmission/paymentDateController';
...
describe('Payment date', () => {
  ...
  describe('on POST', () => {
    it('should redirect when save succeeds for a valid future date', async () => {
      req.body = {year: '9999', month: '1', day: '1'};

      await postHandler(req as AppRequest, res as unknown as Response, next);

      expect(mockSavePaymentDate).toHaveBeenCalledWith(claimId, expect.any(Date), ResponseType.PART_ADMISSION);
    ...
    it('should re-render with error on date in the past', async () => {
      ...
      expect(renderedForm().errorFor('date')).toBe('ERRORS.FIRST_PAYMENT_MESSAGE');
    });

    it('should re-render with error on incorrect input', async () => {
