...
describe('Consider Claimant Documents Controller', () => {
  ...
  describe('on GET', () => {
    it('should return consider claimant documents page', async () => {
      ...
      await request(app).get(DQ_CONSIDER_CLAIMANT_DOCUMENTS).expect((res) => {
    ...
    it('should return status 500 when error thrown', async () => {
      ...
      await request(app)
        .get(DQ_CONSIDER_CLAIMANT_DOCUMENTS)
        .expect((res: Response) => {
  ...
  describe('on POST', () => {
    ...
    it('should return consider claimant documents page', async () => {
      await request(app).post(DQ_CONSIDER_CLAIMANT_DOCUMENTS).expect((res) => {
        expect(res.status).toBe(200);
        expect(res.text).toContain('Are there any documents the claimant has that you want the court to consider?');
      });
    });
    ...
    it('should redirect to the use expert evidence page if option yes is selected', async () => {
      await request(app).post(DQ_CONSIDER_CLAIMANT_DOCUMENTS).send({option: 'yes', details : 'details'})
        .expect((res) => {
          expect(res.status).toBe(302);
          expect(res.get('location')).toBe(DQ_DEFENDANT_EXPERT_EVIDENCE);
        });
    });

    it('should redirect to the use expert evidence page if option no is selected', async () => {
      await request(app).post(DQ_CONSIDER_CLAIMANT_DOCUMENTS).send({option: 'no'})
        .expect((res) => {
          expect(res.status).toBe(302);
          expect(res.get('location')).toBe(DQ_DEFENDANT_EXPERT_EVIDENCE);
        });
    ...
    it('should show errors when no option is selected', async () => {
      await request(app)
        .post(DQ_CONSIDER_CLAIMANT_DOCUMENTS)
        .expect((res: Response) => {
          expect(res.status).toBe(200);
