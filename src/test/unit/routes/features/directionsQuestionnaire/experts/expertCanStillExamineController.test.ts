...
describe('Defendant expert can still examine Controller', () => {
  ...
  describe('on GET', () => {
    it('should return expert can still examine page', async () => {
      ...
      await request(app).get(DQ_DEFENDANT_CAN_STILL_EXAMINE_URL).expect((res) => {
    ...
    it('should return status 500 when error thrown', async () => {
      ...
      await request(app)
        .get(DQ_DEFENDANT_CAN_STILL_EXAMINE_URL)
        .expect((res) => {
  ...
  describe('on POST', () => {
    ...
    it('should return expert can still examine page', async () => {
      await request(app).post(DQ_DEFENDANT_CAN_STILL_EXAMINE_URL).expect((res) => {
        expect(res.status).toBe(200);
        expect(res.text).toContain('Does the claim involve something an expert can still examine?');
      });
    });
    ...
    it('should redirect to the defendant expert reports page if option yes is selected and reason is provided', async () => {
      await request(app).post(DQ_DEFENDANT_CAN_STILL_EXAMINE_URL)
    ...
    it('should return status 500 when error thrown', async () => {
      app.locals.draftStoreClient = mockRedisFailure;
      await request(app)
        .post(DQ_DEFENDANT_CAN_STILL_EXAMINE_URL)
        .send({option: 'no'})
        .expect((res) => {
          expect(res.status).toBe(500);
          expect(res.text).toContain(TestMessages.SOMETHING_WENT_WRONG);
