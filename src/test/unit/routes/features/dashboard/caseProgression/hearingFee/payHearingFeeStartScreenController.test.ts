...
describe('Pay Hearing Fee Start Screen Controller', () => {
  ...
  it('should return expected pay hearing fee page when claim exists', async () => {
    ...
    //When
    await testSession.get(CP_PAY_HEARING_FEE_START_URL.replace(':id', claimId))
    //Then
      .expect((res: { status: unknown; text: unknown; }) => {
        expect(res.status).toBe(200);
        expect(res.text).toContain(t('PAGES.PAY_HEARING_FEE.START.TITLE'));
      });
  });
  ...
  it('should return "Something went wrong" page when claim does not exist', async () => {
    ...
    //When
    await testSession.get(CP_PAY_HEARING_FEE_START_URL.replace(':id', '1111'))
    //Then
      .expect((res: { status: unknown; text: unknown; }) => {
