...
const stringWithLeadingAndTrailingSpaces = '  Address Line 1  ';
...
describe(('For Address Form'), () => {
  ...
  it('should not throw error if address length OK, contains spaces and flag ON', async () => {
    //Given
    jest.spyOn(launchDarkly, 'isJudgmentOnlineLive').mockResolvedValue(true);
    const address = new Address(stringWithLeadingAndTrailingSpaces, stringWithLeadingAndTrailingSpaces
      , stringWithLeadingAndTrailingSpaces, stringWithLeadingAndTrailingSpaces, postCode);
    const form = new GenericForm(address);
    //When
    await form.validate();
    //Then
