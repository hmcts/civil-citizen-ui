...
describe('ViewQueriesService', () => {
  ...
  const claimantCreatedBy = `${claimantUser.id}::[CLAIMANT]`;
  const defendantCreatedBy = `${defendantUser.id}::[DEFENDANT]`;
  const caseworkerCreatedBy = `${caseworkerUser.id}::[CASEWORKER]`;
