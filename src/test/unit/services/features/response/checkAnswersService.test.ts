...
const PAGES_CHECK_YOUR_ANSWER_OWE_MONEY = 'PAGES.CHECK_YOUR_ANSWER.OWE_MONEY';
const COMMON_RESPONSE_TYPE_FULL_ADMISSION = 'COMMON.RESPONSE_TYPE.FULL_ADMISSION';
...
describe('Check Answers service', () => {
  describe('Get Summary Sections', () => {
    ...
    it('should return response to claim when financial detail section exists with payment option type instalments', async () => {
      ...
      //Then
      expect(summarySections.sections[INDEX_RESPONSE_CLAIM_SECTION].summaryList.rows[0].key.text).toBe(PAGES_CHECK_YOUR_ANSWER_OWE_MONEY);
      expect(summarySections.sections[INDEX_RESPONSE_CLAIM_SECTION].summaryList.rows[0].value.html).toBe(COMMON_RESPONSE_TYPE_FULL_ADMISSION);
      expect(summarySections.sections[INDEX_RESPONSE_CLAIM_SECTION].summaryList.rows[0].actions?.items[0].href).toBe(CITIZEN_RESPONSE_TYPE_URL.replace(':id', CLAIM_ID));
