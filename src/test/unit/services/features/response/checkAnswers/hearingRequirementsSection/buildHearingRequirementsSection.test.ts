...
describe('Hearing Requirements Section', () => {
  ...
  describe('FastTrack Claim ', () => {
    ...
    it('build hearing requirement for Fast Track Claim When all questions answer set to yes.', () => {
      ...
      expect(summaryRows.summaryList.rows[7].value.html).toEqual('');
      expect(summaryRows.summaryList.rows[8].key.text).toEqual('PAGES.CHECK_YOUR_ANSWER.GIVE_EVIDENCE');
      ...
      expect(summaryRows.summaryList.rows[12].value.html).toEqual('Test Phone or video hearing');
      expect(summaryRows.summaryList.rows[13].key.text).toEqual('PAGES.CHECK_YOUR_ANSWER.VULNERABILITY_QUESTION');
      ...
      expect(summaryRows.summaryList.rows[16].value.html).toEqual(null);
    ...
    it('build hearing requirement for Fast Track Claim When all questions answer set to no.', () => {
      ...
      expect(summaryRows.summaryList).toStrictEqual('test')
