...
describe('Other Dependants Details', () => {
  it('should return carer other dependants declaration to "yes" when it exists', async () => {
    ...
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[6].value.html).toBe('Yes');
    ...
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[7].actions?.items[0].href).toBe(CITIZEN_OTHER_DEPENDANTS_URL.replace(':id', constVal.CLAIM_ID));
    ...
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[8].actions?.items[0].href).toBe(CITIZEN_OTHER_DEPENDANTS_URL.replace(':id', constVal.CLAIM_ID));
