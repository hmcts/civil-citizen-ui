...
describe('Priority Debts Details', () => {
  it('should return priority debts when it exists', async () => {
    ...
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[16].key.text).toBe('3. ' + constVal.PAGES_CHECK_YOUR_ANSWER_PRIORITY_DEBT_TYPE);
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[16].value.html).toBe('PAGES.CHECK_YOUR_ANSWER.PRIORITY_DEBT_COUNCIL_TAX');
    ...
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[22].key.text).toBe('6. ' + constVal.PAGES_CHECK_YOUR_ANSWER_PRIORITY_DEBT_TYPE);
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[22].value.html).toBe('PAGES.CHECK_YOUR_ANSWER.PRIORITY_DEBT_WATER');
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[23].key.text).toBe(constVal.PAGES_CHECK_YOUR_ANSWER_PRIORITY_DEBT_ARREARS_REPAYMENT);
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[23].value.html).toBe('£500');

    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[24].key.text).toBe('7. ' + constVal.PAGES_CHECK_YOUR_ANSWER_PRIORITY_DEBT_TYPE);
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[24].value.html).toBe('PAGES.CHECK_YOUR_ANSWER.PRIORITY_DEBT_MAINTENANCE');
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[25].key.text).toBe(constVal.PAGES_CHECK_YOUR_ANSWER_PRIORITY_DEBT_ARREARS_REPAYMENT);
    expect(summarySections.sections[constVal.INDEX_FINANCIAL_SECTION].summaryList.rows[25].value.html).toBe('£500');
  });
});
