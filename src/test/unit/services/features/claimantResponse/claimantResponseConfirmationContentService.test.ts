...
describe('Claimant Response Confirmation service', () => {
  ...
  it('Dispute Scenario when claimant intention is not to proceed with claim', () => {
    ...
    // When
    const claimantResponseConfirmationContent = getClaimantResponseConfirmationContent(claim, lang, false, false);
    // Then
  ...
  it('Claimant accepted defendant`s response as part admit pay immediately', () => {
    ...
    // When
    const claimantResponseConfirmationContent = getClaimantResponseConfirmationContent(claim, lang, false, false, new Date());
    // Then
  ...
  it('Claimant rejected defendant`s response as full defence states paid and says not settle - intermediate track enabled', () => {
    ...
    // When
    const claimantResponseConfirmationContent = getClaimantResponseConfirmationContent(claim, lang, true, true);
    ...
    expect(claimantResponseConfirmationContent[0].data?.html).toContain('000MC009');
    expect(claimantResponseConfirmationContent[0].data?.html).toContain(formatDateToFullDate(new Date()));
    expect(claimantResponseConfirmationContent[1].data?.text).toEqual('PAGES.CLAIMANT_RESPONSE_CONFIRMATION.REJECTED_DEFENDANT_RESPONSE.MINTI.WHAT_HAPPENS_NEXT_TEXT_PARA_1');
    expect(claimantResponseConfirmationContent[2].data?.text).toEqual('PAGES.SUBMIT_CONFIRMATION.WHAT_HAPPENS_NEXT');
    expect(claimantResponseConfirmationContent[3].data?.text).toEqual('PAGES.CLAIMANT_RESPONSE_CONFIRMATION.REJECTED_DEFENDANT_RESPONSE.MINTI.REVIEW_CASE');
    expect(claimantResponseConfirmationContent[6]).toBeUndefined();
  });
