...
function addTotalAmountAndYesHearing(claim: Claim) {
  ...
  claim.claimantResponse.directionQuestionnaire.hearing = createHearing(YesNo.YES, YesNo.YES, YesNo.YES, YesNo.YES, undefined, YesNo.YES, YesNo.YES);
}
...
describe('Hearing Requirements Section', () => {
  ...
  describe('FastTrack Claim ', () => {
    ...
    it('should build hearing requirement for Fast Track Claim When all questions answer set to no.', () => {
      ...
      claim.claimantResponse=new ClaimantResponse();
      claim.claimantResponse.directionQuestionnaire=new DirectionQuestionnaire();
      claim.claimantResponse.directionQuestionnaire.hearing = hearing;
      ...
      //Then
      expect(summaryRows.summaryList.rows.length).toEqual(9);
      expect(summaryRows.title).toEqual('PAGES.CHECK_YOUR_ANSWER.HEARING_REQUIREMENTS_TITLE');
      expect(summaryRows.summaryList.rows[0].key.text).toEqual('PAGES.CHECK_YOUR_ANSWER.TRIED_TO_SETTLE');
    ...
    it('should not display triedToSettleRow when tried to settle is not set', ()=> {
      ...
      claim.claimantResponse=new ClaimantResponse();
      claim.claimantResponse.directionQuestionnaire=new DirectionQuestionnaire();
      claim.claimantResponse.directionQuestionnaire.hearing =  createHearing(undefined, YesNo.NO, YesNo.NO, YesNo.NO, undefined);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
    ...
    it('should not display request extra 4 weeks section when no answer for that is set', () => {
      ...
      claim.claimantResponse=new ClaimantResponse();
      claim.claimantResponse.directionQuestionnaire=new DirectionQuestionnaire();
      claim.claimantResponse.directionQuestionnaire.hearing =  createHearing(YesNo.NO, undefined, YesNo.NO, YesNo.NO, undefined);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
      expect(summaryRows.summaryList.rows.length).toEqual(8);
    ...
    it('should not display consider claimant documents section when the answer is not provided', () =>{
      //Given
      claim.claimantResponse.directionQuestionnaire.hearing =  createHearing(YesNo.NO, YesNo.NO, undefined, YesNo.NO, undefined);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
    ...
    it('should  display expert evidence section when there is data for that section', () => {
      ...
      claim.claimantResponse.directionQuestionnaire.experts.expertEvidence = new GenericYesNo(YesNo.YES);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
    ...
    it('should not display expert evidence section when there is no data for that section', () => {
      ...
      addTotalAmountAndYesHearing(claim);
      claim.claimantResponse.directionQuestionnaire.experts = new Experts();
      claim.claimantResponse.directionQuestionnaire.experts.expertEvidence = undefined;
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
      expect(summaryRows.summaryList.rows.length).toEqual(19);
    ...
    it('should display expert reports when there is data for that section', () => {
      ...
      addTotalAmountAndYesHearing(claim);
      claim.claimantResponse.directionQuestionnaire.experts = new Experts();
      claim.claimantResponse.directionQuestionnaire.experts.sentExpertReports = new SentExpertReports(YesNoNotReceived.NO);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
      expect(summaryRows.summaryList.rows.length).toEqual(20);
    ...
    it('should display shared expert section when there is data for that section', () => {
      ...
      addTotalAmountAndYesHearing(claim);
      claim.claimantResponse.directionQuestionnaire.experts = new Experts();
      claim.claimantResponse.directionQuestionnaire.experts.sharedExpert = new GenericYesNo(YesNo.YES);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
      expect(summaryRows.summaryList.rows.length).toEqual(20);
    ...
    it('should display experts when claim has experts', () => {
      ...
      claim.claimantResponse.directionQuestionnaire.experts.expertEvidence = new GenericYesNo(YesNo.YES);
      claim.claimantResponse.directionQuestionnaire.experts.expertDetailsList = new ExpertDetailsList([new ExpertDetails('John', 'Smith', 'email', 60098, 'reason', 'expert', 1000)]);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
      expect(summaryRows.summaryList.rows.length).toEqual(4);
    });
    it('should not display experts when claim does not have experts', () => {
      //Given
      claim.totalClaimAmount = 11000;
      claim.claimantResponse.directionQuestionnaire.experts = new Experts();
      claim.claimantResponse.directionQuestionnaire.experts.expertEvidence = new GenericYesNo(YesNo.NO);
      //When
      const summaryRows = buildHearingRequirementsSection(claim, '1', 'eng');
      //Then
  ...
  describe('Small Claim', () => {
    ...
    it('should build hearing requirement for Small Claim.', () => {
      ...
      claim.claimantResponse.directionQuestionnaire = new DirectionQuestionnaire();
      claim.claimantResponse.directionQuestionnaire.hearing = createHearing(undefined, undefined, undefined, YesNo.YES, YesNo.NO, YesNo.NO, YesNo.NO);
      claim.claimantResponse.directionQuestionnaire.defendantYourselfEvidence = {
        option: YesNo.YES,
      };
      claim.claimantResponse.directionQuestionnaire.vulnerabilityQuestions = new VulnerabilityQuestions();
      ...
      expect(summaryRows.summaryList.rows[12].value.html).toEqual('COMMON.NO');
