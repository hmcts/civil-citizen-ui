...
import {ELIGIBILITY_TENANCY_DEPOSIT_URL} from '../../../../../../main/routes/urls';
...
describe('Eligibility Tenancy Deposit View', () => {
  ...
  describe('on GET', () => {
    ...
    beforeEach(async () => {
      ...
      const response = await request(app).get(ELIGIBILITY_TENANCY_DEPOSIT_URL);
      const dom = new JSDOM(response.text);
      htmlDocument = dom.window.document;
    });

    it('should have correct page title', () => {
      expect(htmlDocument.title).toEqual('Your money claims account - Is your claim for a tenancy deposit?');
    });

    it('should display header', () => {
      const header = htmlDocument.getElementsByClassName('govuk-heading-l');
      expect(header[0].innerHTML).toContain('Is your claim for a tenancy deposit?');
    });
  ...
  describe('on POST', () => {
    ...
    beforeEach(async () => {
      nock(idamUrl)
        .post('/o/token')
        .reply(200, {id_token: citizenRoleToken});
      const response = await request(app).post(ELIGIBILITY_TENANCY_DEPOSIT_URL);
      const dom = new JSDOM(response.text);
      htmlDocument = dom.window.document;
    });
