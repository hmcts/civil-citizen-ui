...
import {ELIGIBILITY_GOVERNMENT_DEPARTMENT_URL} from '../../../../../../main/routes/urls';
...
describe('Claim Against Government View', () => {
  ...
  describe('on GET', () => {
    ...
    beforeEach(async () => {
      ...
      const response = await request(app).get(ELIGIBILITY_GOVERNMENT_DEPARTMENT_URL);
      const dom = new JSDOM(response.text);
      htmlDocument = dom.window.document;
    });

    it('should have correct page title', () => {
      expect(htmlDocument.title).toEqual('Your money claims account - Government department');
    });

    it('should display header', () => {
      const header = htmlDocument.getElementsByClassName('govuk-heading-l');
      expect(header[0].innerHTML).toContain('Are you claiming against a government department?');
    });

    it('should display link', () =>{
      const link = htmlDocument.getElementById('govuk-gov-list');
      expect(link?.innerHTML).toContain('See list of government departments (opens in a new window).');
    });
  ...
  describe('on POST', () => {
    ...
    beforeEach(async () => {
      nock(idamUrl)
        .post('/o/token')
        .reply(200, {id_token: citizenRoleToken});
      const response = await request(app).post(ELIGIBILITY_GOVERNMENT_DEPARTMENT_URL);
      const dom = new JSDOM(response.text);
      htmlDocument = dom.window.document;
    });
