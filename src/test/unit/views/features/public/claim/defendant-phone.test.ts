...
describe('Defendant Phone View', () => {
  ...
  describe('on GET', () => {
    ...
    beforeEach(async () => {
      ...
      const response = await request(app).get(DEFENDANT_PHONE_NUMBER_URL);
    ...
    it('should have correct page title', () => {
      expect(htmlDocument.title).toEqual(`Your money claims account - ${t('PAGES.DEFENDANT_PHONE_NUMBER.PAGE_TITLE')}`);
    });

    it('should display heading', () => {
      const body = mainWrapper.getElementsByClassName('govuk-heading-l');
      expect(body[0].innerHTML).toContain(t('PAGES.DEFENDANT_PHONE_NUMBER.TITLE'));
    });
