    ...
    describe('on POST', () => {
      ...
      it('should display correct error summary message with correct link for comment greater than max length', async () => {
        await request(app)
          ...
          .send({
            comment: tooLongEvidenceDetails,
            evidenceItem: [{type: EvidenceType.CONTRACTS_AND_AGREEMENTS, description: 'content here..'}],
          })
          .then(res => {
            const dom = new JSDOM(res.text);
            htmlDocument = dom.window.document;
      ...
      it('should display correct error summary message with correct link for description greater than max length', async () => {
        await request(app)
          ...
          .send({
            comment: '',
            evidenceItem: [{type: EvidenceType.CONTRACTS_AND_AGREEMENTS, description: tooLongEvidenceDetails}],
          })
          .then(res => {
            const dom = new JSDOM(res.text);
            htmlDocument = dom.window.document;
