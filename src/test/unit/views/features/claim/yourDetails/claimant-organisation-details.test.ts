...
import {CLAIMANT_COMPANY_DETAILS_URL, CLAIMANT_ORGANISATION_DETAILS_URL} from '../../../../../../main/routes/urls';
...
describe('Claimant Organisation Details View', () => {
  ...
  describe('Organisation Type', () => {
    ...
    it('should display Organisation name and Contact person labels', () => {
      const labels = mainWrapper.getElementsByClassName('govuk-label');
      expect(labels[0].innerHTML).toContain('Organisation name');
  ...
  describe('Company type', () => {
    ...
    beforeAll(async () => {
      const response = await request(app).get(CLAIMANT_COMPANY_DETAILS_URL);
      const dom = new JSDOM(response.text);
      htmlDocument = dom.window.document;
      mainWrapper = htmlDocument.getElementsByClassName('govuk-main-wrapper')[0];
    });
    ...
    it('should display correct header', () => {
      const header = htmlDocument.getElementsByClassName('govuk-heading-l');
      expect(header[0].innerHTML).toContain(t('PAGES.COMPANY_DETAILS.CLAIMANT_TITLE'));
    });
    ...
    describe('Your postal address section', () => {
      it('should display Your postal address heading', () => {
        const header = mainWrapper.getElementsByClassName('govuk-heading-m')[0];
        expect(header.innerHTML).toContain(t('PAGES.COMPANY_DETAILS.COMPANY_ADDRESS'));
      });
