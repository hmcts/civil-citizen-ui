...
Scenario('Response with RejectAll and AlreadyPaid @citizenUI @rejectAll @park @nightly', async ({api}) => {
...
Scenario('Response with RejectAll and DisputeAll @citizenUI @rejectAll @park @nightly', async ({api}) => {
  await ResponseSteps.RespondToClaim(claimRef);
  await ResponseSteps.EnterPersonalDetails(claimRef);
  await ResponseSteps.EnterYourOptionsForDeadline(claimRef, dontWantMoreTime);
  await ResponseSteps.EnterResponseToClaim(claimRef, rejectAll);
