...
Scenario('Response with RejectAll and AlreadyPaid Welsh @citizenUI @rejectAll @nightly', async ({api}) => {
  ...
  await ResponseSteps.EnterFreeTelephoneMediationDetails(claimRef,'cy');
...
Scenario('Response with RejectAll and DisputeAll Welsh @citizenUI @rejectAll @nightly', async ({api}) => {
  await ResponseSteps.RespondToClaim(claimRef);
  await ResponseSteps.EnterPersonalDetails(claimRef);
  await ResponseSteps.EnterYourOptionsForDeadline(claimRef, dontWantMoreTime);
  await ResponseSteps.EnterResponseToClaim(claimRef, rejectAll);
