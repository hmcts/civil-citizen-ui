...
Scenario('Response with AdmitAll and Date to PayOn @citizenUI @admitAll @park @nightly', async ({api}) => {
...
Scenario('Response with AdmitAll and Repayment plan @citizenUI @admitAll @park @nightly', async ({api}) => {
  await ResponseSteps.RespondToClaim(claimRef);
  await ResponseSteps.EnterPersonalDetails(claimRef);
  await ResponseSteps.EnterYourOptionsForDeadline(claimRef, dontWantMoreTime);
  await ResponseSteps.EnterResponseToClaim(claimRef, admitAll);
