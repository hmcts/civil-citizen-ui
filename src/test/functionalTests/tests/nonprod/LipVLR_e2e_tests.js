...
Scenario('LipVLR - NoC and DefendantLR respond as DefenceAll @citizenUI', async ({
  ...
  if (['preview', 'demo'].includes(config.runningEnv)) {
    await testTimeHelper.addTestStartTime('LipVLR - NoC and DefendantLR respond as DefenceAll');
    selectedHWF = false;
    const defaultClaimFee = 455;
    const defaultClaimAmount = 9000;
    isDashboardServiceEnabled = await isDashboardServiceToggleEnabled();
    ...
    await testTimeHelper.addTestEndTime('LipVLR - NoC and DefendantLR respond as DefenceAll');
  }
});
...
Scenario('LipVLR - NoC and DefendantLR respond as AdmitAll @citizenUI', async ({
  ...
  if (['preview', 'demo'].includes(config.runningEnv)) {
    await testTimeHelper.addTestStartTime('LipVLR - NoC and DefendantLR respond as AdmitAll');
    selectedHWF = false;
    const defaultClaimFee = 455;
    const defaultClaimAmount = 9000;
    isDashboardServiceEnabled = await isDashboardServiceToggleEnabled();
    ...
    await testTimeHelper.addTestEndTime('LipVLR - NoC and DefendantLR respond as AdmitAll');
  }
});
...
Scenario('LipVLR - NoC and DefendantLR respond as PartAdmit @citizenUI - @api @noc @nightly', async ({
  ...
  if (['preview', 'demo'].includes(config.runningEnv)) {
    await testTimeHelper.addTestStartTime('LipVLR - NoC and DefendantLR respond as PartAdmit');
    selectedHWF = false;
    const defaultClaimFee = 455;
    const defaultClaimAmount = 9000;
    isDashboardServiceEnabled = await isDashboardServiceToggleEnabled();
    ...
    await testTimeHelper.addTestEndTime('LipVLR - NoC and DefendantLR respond as PartAdmit');
  }
});
...
Scenario('LipVLR - NoC and DefendantLR respond as CounterClaim @citizenUI - @api @noc @nightly', async ({
  ...
  if (['preview', 'demo'].includes(config.runningEnv)) {
    await testTimeHelper.addTestStartTime('LipVLR - NoC and DefendantLR respond as CounterClaim');
    selectedHWF = false;
    const defaultClaimFee = 455;
    const defaultClaimAmount = 9000;
    isDashboardServiceEnabled = await isDashboardServiceToggleEnabled();
    ...
    await testTimeHelper.addTestEndTime('LipVLR - NoC and DefendantLR respond as CounterClaim');
  }
});
