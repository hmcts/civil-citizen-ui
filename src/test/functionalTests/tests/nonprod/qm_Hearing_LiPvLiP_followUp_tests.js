...
Feature('QM - LIP - Follow up tests @local-testing');
...
Scenario('Claimant and Defendant send message to court and follow up and admin closes query', async ({ api, qm, I }) => {
  ...
  const initialQuery = await raiseAndRespondToQueries(
    qm,
    caseId,
  ...
  await closeQuery(qm, caseId, config.ctscAdmin, PUBLIC_QUERY, initialQuery);
});
