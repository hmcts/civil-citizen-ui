...
Feature('Response with PartAdmit-PayImmediately - Small Claims & Fast Track').tag('@civil-citizen-nightly');

Scenario.skip('Response with PartAdmit-PayImmediately Small claims', async ({api}) => {
...
Scenario.skip('Response with PartAdmit-PayImmediately Fast Track', async ({api}) => {
  await createAccount(config.claimantCitizenUser.email, config.claimantCitizenUser.password);
  await createAccount(config.defendantCitizenUser.email, config.defendantCitizenUser.password);
  claimType = 'FastTrack';
  claimRef = await api.createLiPClaim(config.claimantCitizenUser, claimType);
