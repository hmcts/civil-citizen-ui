...
class ViewDocuments {
  ...
  async verifyHeadingDetails() {
    await I.see('Orders and notices from the court', 'span');
  ...
  async checkRequestToReviewOrder(partyType, date) {
    ...
    await I.see('Request to review order', requestReviewOrderLocator);
    await I.see ('Created ['+ date +']', requestReviewOrderLocator);
    await I.see(partyType + '_request_for_reconsideration.pdf', requestReviewOrderLocator);
