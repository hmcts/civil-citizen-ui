...
class ConfirmationPage {
  async verifyConfirmationPage(claimType) {
    await I.waitForContent('What happens next', config.WaitForText);
    switch (claimType) {
      case 'RejectsAndLessThanClaimAmount': {
        await I.see('You told us you\'ve paid the £500 you believe you owe. ' +
          'We\'ve sent Test Company Claimant this response.');
        await I.see('If Test Company Claimant accepts your response');
        await I.see('The claim will be settled');
        await I.see('If Test Company Claimant rejects your response');
        break;
      }
      case 'PartAdmitAndPayImmediately': {
        await I.see('You\'ve said you owe £500.00 plus the claim fee and any fixed costs claimed and offered to pay Miss Jane Doe immediately.');
        break;
      }
    }
    await I.see('The next step will be mediation. ' +
      'The mediation service will contact you to give you a date for your appointment.');
    await I.see('If you cannot reach an agreement at mediation, the court will review the case.');
  }
