import {config} from '/Users/nishanth/Code/MOJ/civil-citizen-ui/src/test/config.ts';
...
import waitForFinishedBusinessProcess from './testingSupport';
import { assignCaseRoleToUser, addUserCaseMapping, unAssignAllUsers } from './caseRoleAssignmentHelper';
import apiRequest from './apiRequest.js';
import claimData from '../fixtures/events/createClaimSpecFast.js';
import expectedEvents from '../fixtures/ccd/expectedEventsLRSpec.js';
...
module.exports = {
  ...
  createClaimWithRepresentedRespondent: async (user,scenario = 'ONE_V_ONE') => {
    ...
    for (const pageId of Object.keys(createClaimData.userInput)) {
  ...
  informAgreedExtensionDate: async (user) => {
    ...
    const informAgreedExtensionData = data.INFORM_AGREED_EXTENSION_DATE();

    for (const pageId of Object.keys(informAgreedExtensionData.userInput)) {
  ...
  defendantResponse: async (user, response = 'FULL_DEFENCE', scenario = 'ONE_V_ONE') => {
    ...
    const returnedCaseData = await apiRequest.startEvent(eventName, caseId);

    const defendantResponseData = eventData['defendantResponses'][scenario][response];

    caseData = returnedCaseData;

    console.log(`${response} ${scenario}`);

    for (const pageId of Object.keys(defendantResponseData.userInput)) {
  ...
  claimantResponse: async (user, response = 'FULL_DEFENCE', scenario = 'ONE_V_ONE',
    ...
    caseData = await apiRequest.startEvent(eventName, caseId);
    const claimantResponseData = eventData['claimantResponses'][scenario][response];

    for (const pageId of Object.keys(claimantResponseData.userInput)) {
