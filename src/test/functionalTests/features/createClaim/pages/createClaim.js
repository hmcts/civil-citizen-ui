...
class CreateClaim {
  ...
  async verifyAndChooseLanguage(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
      ...
      //TO DO other welsh  checks
      I.click('#option-2');
    }
    I.click('Save and continue');
  }

  async verifyTaskListPage(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyTryToResolveTheDispute(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyCompletingYourClaim(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  verifyAboutYouAndThisClaimForClaimant(selectedLang, claimantType) {
    if (selectedLang == language.ENGLISH) {
    ...
    switch (claimantType) {
      case userPartyType.INDIVIDUAL:
        I.click(paths.options.individual_claimant);
        break;
      case userPartyType.SOLE_TRADER:
        I.click(paths.options.SOLE_TRADER);
        break;
      case userPartyType.LIMITED_COMPANY:
        I.click(paths.options.LIMITED_COMPANY);
        break;
      case userPartyType.ORG:
        I.click(paths.options.ORG);
        break;
    }
    this.clickNextAction(paths.buttons.save_and_continue);
  }

  verifyAboutYouAndThisClaimForDefendant(selectedLang, defendantType) {
    if (selectedLang == language.ENGLISH) {
      I.waitForText('For example a partnership, trust, charity, club or association', 60);
    ...
    switch (defendantType) {
      case userPartyType.INDIVIDUAL:
        ...
        break;
      case userPartyType.SOLE_TRADER:
        I.click(paths.options.sole_trader_defendant);
        break;
      case userPartyType.LIMITED_COMPANY:
        I.click(paths.options.limited_company_defendant);
        break;
      case userPartyType.ORG:
        I.click(paths.options.org_defendant);
        break;
    }
    this.clickNextAction(paths.buttons.save_and_continue);
  }

  verifyEnterYourDetails(selectedLang = language.ENGLISH, userPartyType) {
    if (selectedLang == language.ENGLISH) {
      ...
      I.see('Enter address manually');
      I.see('Find address');
      if (userPartyType == userPartyType.INDIVIDUAL) {
        I.see('Enter company details', 'h1');
        I.see('Title');
        I.see('First name');
        I.see('Last name');
        I.see('Your postal address', 'h2');
      } else if (userPartyType == userPartyType.SOLE_TRADER) {
        I.see('Enter your details', 'h1');
        I.see('Title');
        I.see('First name');
        I.see('Last name');
        I.see('Your postal address', 'h2');
        I.see('Trading (optional)');
      } else if (userPartyType == userPartyType.LIMITED_COMPANY) {
        I.see('Enter company details', 'h1');
        I.see('Company name');
        I.see('Contact person (optional)');
        I.see('Company Address', 'h2');
        I.see('Enter the company\'s registered office or a company address that has a connection with the claim.');
      } else {
        I.see('Enter organisation details', 'h1');
        I.see('Organisation name');
        I.see('Contact person (optional)');
        I.see('Organisation Address', 'h2');
        I.see('Enter the organisation\'s main office or an organisation\'s address that has a connection with the claim.');
      }
    }
  }
  ...
  inputEnterYourDetails(isAddingClaimantDetails, selectedLang, userPartyType) {
    if (selectedLang == language.ENGLISH) {
      if (isAddingClaimantDetails === true) {
      ...
      } else if (isAddingClaimantDetails === false) {
  ...
  async verifyContentOnClaimAmountScreen(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyAndInputDoYouWantToClaimInterest(addInterest, selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
      I.waitForText('No', 60);
  ...
  async verifyAndInputHowDoYouWantToClaimInterest(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyAndInputWhatAnnualRateOfInterestDoYouWantToClaim(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyAndInputWhenWillYouClaimInterestFrom(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyAndInputHelpWithFees(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async inputClaimAmount(selectedLang = language.ENGLISH) {
  ...
  async verifyClaimAmountSummary(addInterest, selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyAndInputClaimDetails(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyClaimDetailsTimeline(selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
      I.waitForText('Year',60);
  ...
  async verifyCheckYourAnswers(addInterest, selectedLang = language.ENGLISH) {
    if (selectedLang == language.ENGLISH) {
  ...
  async verifyClaimSubmitted(selectedLang = language.ENGLISH) {
    let claimReference;
    if (selectedLang == language.ENGLISH) {
      I.waitForText('Monday to Friday, 8.30am to 5pm.', 60);
      I.see('Claim submitted', 'h1');
      I.see('Claim number:');
      claimReference = await I.grabTextFrom('//div[contains(text(),\'Claim number\')]/strong');
