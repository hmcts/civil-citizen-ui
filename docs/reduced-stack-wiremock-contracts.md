...
| Submit draft | `submitDraftClaim` / `submitEvent` | POST | `/cases/draft/citizen/{submitterId}/event` | None | `Content-Type: application/json`; Civil Service auth | `EventDto`; `event=CREATE_LIP_CLAIM` | 200 | `id`, `state`, `last_modified`, mandatory `case_data` used by conversion | `create-claim-submitted-claim.json` | `create-claim-submit.json` | CUI | Four translated party-pairing interactions and matching provider states in DTSCCI-6413; see [Pact inventory](../src/test/contract/interaction-inventory.json) |
| Read submitted claim | `getClaimById` | GET | `/cases/1111222233334444` | None | Civil Service auth | None | 200 | Same representative case fields as submit | `create-claim-submitted-claim.json` | `create-claim-read.json` | CUI | Uncovered; DTSCCI-6414 |
| Resolve case roles | `getUserCaseRoles` | GET | `/cases/1111222233334444/userCaseRoles` | None | Civil Service auth | None | 200 | Role string array | `create-claim-user-case-roles.json` | `create-claim-read.json` | CUI | Uncovered; DTSCCI-6414 |
