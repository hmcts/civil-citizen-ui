...
locals {
  ...
  alert_secrets = length(var.monitor_action_group) > 0 ? ["civil-ci-alert-slack-group-webhook"] : []
}
...
data "azurerm_key_vault_secret" "alert_secrets" {
  for_each = toset(local.alert_secrets)
...
resource "azurerm_key_vault_secret" "alert_secrets" {
  for_each = data.azurerm_key_vault_secret.alert_secrets
